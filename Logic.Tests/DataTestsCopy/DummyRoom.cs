...
namespace TPUM.Logic.Tests
{
    internal class DummyRoom : IRoom
    {
        ...
        public DummyRoom(Data.IRoom room)
        {
            _room = room;
        ...
        public IHeater AddHeater(float x, float y, float temperature)
        {
            ...
            var heater = new DummyHeater(_room.AddHeater(x, y, temperature));
        ...
        public IHeatSensor AddHeatSensor(float x, float y)
        {
            ...
            var sensor = new DummyHeatSensor(_room.AddHeatSensor(x, y));
        ...
        private void UpdateTemperature(float deltaTime)
        {
            lock (_heatersLock)
            {
                ...
                lock (_heatSensorsLock)
                {
                    ...
                    foreach (var sensor in _heatSensors)
                    {
                        (sensor as DummyHeatSensor)?.SetTemperature(MathF.Max(sensor.Temperature - TemperatureDecayFactor * deltaTime, 0f));

                        if (onHeaters.Count == 0) continue;
                        foreach (var tempDiff in from heater in onHeaters
                                                 let dist = IPosition.Distance(sensor.Position, heater.Position)
                                                 select MathF.Min(heater.Temperature, heater.Temperature * MathF.Exp(-TemperatureDecayFactor * dist)) * deltaTime)
                        {
                            (sensor as DummyHeatSensor)?.SetTemperature(sensor.Temperature + tempDiff -
                                                                   TemperatureDecayFactor * deltaTime);
                        }
                        (sensor as DummyHeatSensor)?.SetTemperature(MathF.Min(sensor.Temperature, onHeaters.Max(heater => heater.Temperature)));
                    }
                }
            }
        }
